...
class Model_Vgg19:
    ...
    def build(self, is_train=True):
        ...
        with slim.arg_scope([slim.conv2d], padding='VALID', weights_initializer=tf.truncated_normal_initializer(stddev=0.01)):
            ...
            net = slim.conv2d(self.images, 32, [3, 3], 1, padding='SAME', activation_fn=tf.nn.relu, scope='conv1_1')
            #net = slim.batch_norm(net, scope='bn1_1')
            net = slim.conv2d(net, 32, [3, 3], 1, padding='SAME', activation_fn=tf.nn.relu, scope='conv1_2')
            #net = slim.batch_norm(net, scope='bn1_2')
            net = slim.max_pool2d(net, [2, 2], scope='pool1')
            net = slim.dropout(net, 0.25, is_training=is_train, scope='drop1')
            print(net.shape)

            net = slim.conv2d(net, 64, [3, 3], 1, padding='SAME', activation_fn=tf.nn.relu, scope='conv2_1')
            #net = slim.batch_norm(net, scope='bn2_1')
            net = slim.conv2d(net, 64, [3, 3], 1, padding='SAME', activation_fn=tf.nn.relu, scope='conv2_2')
